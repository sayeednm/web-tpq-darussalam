                ...
                <div className="relative flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-emerald-100 text-sm font-medium">Tim Pengajar</span>
                    </div>
                  ...
                  <button onClick={() => setShowGuru(false)} className="bg-white/20 hover:bg-white/30 p-2 rounded-xl transition flex-shrink-0">
                    <X size={18} />
                  </button>
                </div>
              </div>
                    ...
                    {guruList.map((g) => (
                      ...
                      >
                        <div className="flex items-center gap-4 mb-3">
                          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-bold flex-shrink-0 shadow-sm ${
                            g.jenisKelamin === 'P' ? 'bg-gradient-to-br from-pink-400 to-rose-500 text-white' : 'bg-gradient-to-br from-emerald-400 to-teal-500 text-white'
                          }`}>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${g.jenisKelamin === 'P' ? 'bg-pink-100 text-pink-700' : 'bg-emerald-100 text-emerald-700'}`}>
                        ...
                        {g.tanggalMasuk && (
                          <p className="text-xs text-gray-400 mt-2">
                            Bergabung {new Date(g.tanggalMasuk).toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}
                          </p>
                        )}
              ...
              <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-6 text-white relative overflow-hidden">
                ...
                <div className="relative flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <Users size={20} />
                      <span className="text-blue-100 text-sm font-medium">Data Santri</span>
                    </div>
                  ...
                  <button onClick={() => setShowSantri(false)} className="bg-white/20 hover:bg-white/30 p-2 rounded-xl transition flex-shrink-0">
                    <X size={18} />
                  </button>
                </div>
              </div>
              ...
              <div className="overflow-y-auto p-5">
                {santriList.length === 0 ? (
                  <div className="py-16 text-center text-gray-400">
                    ...
                    <p className="font-medium">Belum ada data santri</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {santriList.map((s, i) => (
                      ...
                      >
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg font-bold mx-auto mb-2 shadow-sm ${
                          s.jenisKelamin === 'P' ? 'bg-gradient-to-br from-pink-400 to-rose-500 text-white' : 'bg-gradient-to-br from-blue-400 to-indigo-500 text-white'
                        }`}>
          ...
          >
            ...
            >
              ...
              <div className={`relative p-8 text-center overflow-hidden ${
                ...
                <button
                  onClick={() => setSelectedGuru(null)}
                  className="absolute top-4 right-4 bg-white/20 hover:bg-white/30 p-1.5 rounded-xl transition"
                >
              ...
              <div className="p-6 space-y-3">
                <div className="flex flex-wrap gap-2 justify-center mb-4">
                  <span className={`text-sm px-3 py-1.5 rounded-full font-semibold ${
                    selectedGuru.jenisKelamin === 'P' ? 'bg-pink-100 text-pink-700' : 'bg-emerald-100 text-emerald-700'
                  }`}>
                    {selectedGuru.jenisKelamin === 'P' ? '👩 Ustadzah' : '👨 Ustadz'}
                  </span>
                  <span className="text-sm bg-amber-100 text-amber-700 px-3 py-1.5 rounded-full font-semibold">
                    🎓 Bersyahadah Ummi
                  </span>
                </div>
                  ...
                  {selectedGuru.tanggalMasuk && (
                    <div className="flex items-center justify-between px-4 py-3">
                      ...
                      <span className="text-sm font-semibold text-gray-800">
                        {new Date(selectedGuru.tanggalMasuk).toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}
                      </span>
                    </div>
                  )}
                ...
                <button
                  onClick={() => setSelectedGuru(null)}
                  className="w-full py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-2xl font-semibold text-sm transition mt-2"
                >
                  Tutup
                </button>
              </div>
